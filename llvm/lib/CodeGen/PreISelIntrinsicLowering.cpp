...
struct PreISelIntrinsicLowering {
  ...
  explicit PreISelIntrinsicLowering(
      ...
      function_ref<DominatorTree &(Function &)> LookupDT_,
...
static bool ExpandMemCpyAsMaskedOp(MemTransferInst *MemTransfer,const TargetTransformInfo &TTI,AssumptionCache &AC,DominatorTree &DT) {
  Value *Len = MemTransfer->getLength();
  if (isa<ConstantInt>(Len) || MemTransfer->isVolatile())
    return false;
  unsigned VecBytes = TTI.getRegisterBitWidth(TargetTransformInfo::RGK_FixedWidthVector).getFixedValue()/8;
  if(VecBytes == 0){
    return false;
  }
  ...
  if (!TTI.isLegalMaskedLoad(VecTy, Align(1), 0) ||!TTI.isLegalMaskedStore(VecTy, Align(1), 0))
    return false;
  ...
  Value *Loaded = B.CreateMaskedLoad(VecTy, MemTransfer->getRawSource(), Align(1),Mask, PoisonValue::get(VecTy));
  ...
  for (User *U : llvm::make_early_inc_range(F.users())) {
    ...
    case Intrinsic::memcpy: {
      ...
      if (shouldExpandMemIntrinsicWithSize(Memcpy->getLength(), TTI)) {
        if (ExpandMemCpyAsMaskedOp(Memcpy,TTI,LookupAC(*ParentFunc),LookupDT(*ParentFunc))){
          Changed = true;
    ...
    case Intrinsic::memmove: {
      ...
      if (shouldExpandMemIntrinsicWithSize(Memmove->getLength(), TTI)) {
        if(ExpandMemCpyAsMaskedOp(Memmove,TTI,LookupAC(*ParentFunc),LookupDT(*ParentFunc))){
          Changed = true;
...
public:
  ...
  bool runOnModule(Module &M) override {
    ...
    auto LookupDT = [this](Function &F) -> DominatorTree &  {
      return this->getAnalysis<DominatorTreeWrapperPass>(F).getDomTree();
    };

    const auto *TM = &getAnalysis<TargetPassConfig>().getTM<TargetMachine>();
    PreISelIntrinsicLowering Lowering(TM, ModuleLibcalls, LookupTTI, LookupTLI,LookupAC,LookupDT);
...
PreISelIntrinsicLoweringPass::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  PreISelIntrinsicLowering Lowering(TM, LibcallLowering, LookupTTI, LookupTLI,LookupAC,LookupDT);
